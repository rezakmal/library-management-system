Public Class BorrowForm
    ...
    Private Sub BorrowForm_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        sql = " SELECT br.`AccessionNo`, `Title`, `Description`,Concat(`Firstname`,' ', `Lastname`) as 'Borrower', `DateBorrowed`, `Purpose`, `DueDate` FROM `tblborrow` br,`tblinventory` b,`tblborrower` bw  WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` ORDER BY BorrowId Desc"
    ...
    Private Sub txt_Search_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_Search.TextChanged
        sql = " SELECT br.`AccessionNo`, `Title`, `Description`, Concat(`Firstname`,' ', `Lastname`) as 'Borrower', `DateBorrowed`, `Purpose`, `DueDate` " &
        " FROM `tblborrow` br,`tblinventory` b,`tblborrower` bw  " &
        " WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` AND (Title Like '%" & txt_Search.Text & "' or br.AccessionNo Like '%" & txt_Search.Text & "%' OR Concat(`Firstname`,' ', `Lastname`)  Like '%" & txt_Search.Text & "') ORDER BY BorrowId Desc"
        reloadDtg(sql, dtg_BlistOfBooks)
    End Sub
    ...
    Private Sub txtAccesionNumBorrow_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtAccesionNumBorrow.TextChanged
        If txtAccesionNumBorrow.Text = "" Then
            ...
            txtpublisher.Clear()
        Else
            sql = "SELECT * FROM tblinventory b, tblcategory c WHERE b.CategoryId=c.CategoryId AND AccessionNo='" & txtAccesionNumBorrow.Text & "'"
            reloadtxt(sql)
            '`AccessionNo`, `Title`, `PublishDate`, `Vendor`, `BookQuantity`
            If dt.Rows.Count > 0 Then
                With dt.Rows(0)
                    txtTitle.Text = .Item("Title")
                    txtDatePublish.Text = .Item("PublishDate")
                    txtpublisher.Text = .Item("Vendor")
                End With
            End If
        End If
    ...
    Private Sub btnNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        sql = " SELECT br.`AccessionNo`, `Title`, `Description`,Concat(`Firstname`,' ', `Lastname`) as 'Borrower',`NoCopies`, `DateBorrowed`, `Purpose`, `DueDate` FROM `tblborrow` br,`tblinventory` b,`tblborrower` bw  WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` ORDER BY BorrowId Desc"
        reloadDtg(sql, dtg_BlistOfBooks)
    End Sub
    ...
    Private Sub btn_Bsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Bsave.Click
        Try
            ...
            Else
                ...
                'updates("UPDATE tblinventory set BookQuantity = BookQuantity - '" _
